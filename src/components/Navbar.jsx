...
import mylogo from"../assets/Hema.png";
import {FiMenu} from"react-icons/fi";

export default function Navbar(){
  const[menuopen, setMenuOpen] = useState(false);
  const[visible, setVisible] = useState(true);
  const[forceVisible, setFroceVisible] = useState(false);
    ...
    <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-6 py-4 z-50 transition-transform duration-300 
      ${visible? "translate-y-0":"-translate-y-full"}`}>
        <div className='flex items-center space-x-4'>
          ...
          <div className='block lg:absolute lg:left-1/2 lg:translate lg:-translate-x-1/2'>
